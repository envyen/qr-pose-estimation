...
parameter so that it can determine pose using the camera parameters.
@param inputCameraImageWidth: The width of camera images used in the camera
calibration
@param inputCameraImageHeight: The height of the camera images used in the
camera calibration
...
/*
This function takes a BGR frame of the appropriate size, scans for a QR code
with an embedded size (recognized decimal formats: ft, in, cm, mm, m), and
stores the pose of the camera (OpenCV format) relative to the coordinate system
of the QR tag in the provided buffer.  If multiple tags are recognized, it will
only return the information for the first.
...
@param inputQRCodeIdentifierBuffer: A buffer to place left text from the QR code
after the dimension information has been removed
@param inputQRCodeDimensionBuffer: A buffer to place size of the QR code in
meters
...
/*
This function takes a grayscale frame of the appropriate size, scans for a QR
code with an embedded size (recognized decimal formats: ft, in, cm, mm, m), and
stores the pose of the camera (OpenCV format) relative to the coordinate system
of the QR tag in the provided buffer.  If multiple tags are recognized, it will
only return the information for the first.
@param inputGrayscaleFrame: The frame to process (should be same size as
calibration)
@param inputCameraPoseBuffer: The buffer to place the 4x4 camera pose matrix in
@param inputQRCodeIdentifierBuffer: A buffer to place left text from the QR code
after the dimension information has been removed
...
bool QRCodeStateEstimator::estimateStateFromGrayscaleFrame(
	...
	returnValue = estimateOneOrMoreStatesFromGrayscaleFrame(
			inputGrayscaleFrame, cameraPosesBuffer, QRCodeIdentifiersBuffer,
...
/*
This function takes a BGR frame of the appropriate size, scans for a QR codes
with an embedded size (recognized decimal formats: ft, in, cm, mm, m), and
stores the poses of the camera (OpenCV format) relative to the different
coordinate systems of the QR tags in the provided buffer.
@param inputBGRFrame: The frame to process (should be same size as calibration)
@param inputCameraPosesBuffer: The buffer to place the 4x4 camera pose matrices
in
@param inputQRCodeIdentifiersBuffer: A buffer to place the text left from each
QR code after the dimension information has been removed
@param inputQRCodeDimensionsBuffer: A buffer to place size of each QR code in
meters
@return: true if it was able to scan a QR code and estimate its pose relative to
it and false otherwise
...
/*
This function takes a grayscale frame of the appropriate size, scans for any QR
codes with an embedded sizes (recognized decimal formats: ft, in, cm, mm, m),
and stores the poses of the camera (OpenCV format) relative to the different
coordinate systems of the QR tags in the provided buffers.
@param inputGrayscaleFrame: The frame to process (should be same size as
calibration)
@param inputCameraPosesBuffer: The buffer to place the 4x4 camera pose matrices
in
...
bool QRCodeStateEstimator::estimateOneOrMoreStatesFromGrayscaleFrame(
	...
	for (zbar::Image::SymbolIterator symbol = zbarFrame.symbol_begin();
			 symbol != zbarFrame.symbol_end(); ++symbol) {
		if (symbol->get_type() != zbar::ZBAR_QRCODE ||
				symbol->get_location_size() != 4) {
			continue;  // Skip if it isn't a QR code or its outline is described by
								 // more than 4 vertices
		}

		double QRCodeDimensionInMeters;
		std::string QRCodeIdentifierString;
		if (extractQRCodeDimensionFromString(symbol->get_data(),
																				 QRCodeDimensionInMeters,
																				 QRCodeIdentifierString) != true) {
		...
		for (int i = 0; i < symbol->get_location_size(); i++) {
			openCVPoints.push_back(
					cv::Point2d(symbol->get_location_x(i), symbol->get_location_y(i)));
		}

		// The center of the coordinate system associated with the QR code is in the
	...
	if (showResultsInWindow) {
		...
		for (zbar::Image::SymbolIterator symbol = zbarFrame.symbol_begin();
			...
			line(bufferFrame,
					 ...
					 cv::Point(symbol->get_location_x(1), symbol->get_location_y(1)),
			...
			line(bufferFrame,
					 cv::Point(symbol->get_location_x(2), symbol->get_location_y(2)),
					 cv::Point(symbol->get_location_x(3), symbol->get_location_y(3)),
...
/*
This function takes a string in the format "dimensionIdentifier" (for example,
"12.0in-FKDJL") and stores the dimension from the string in meters and the
remainder.  In the example case, it would store 0.3048 and "FKDJL".  It supports
the following extensions and is case insensitive: "m-", "cm-", "mm-", "ft-",
"in-".
@param inputQRCodeString: The original string
@param inputDimensionBuffer: The buffer to store the extracted dimension (in
meters) in
@param inputIdentifierBuffer: The remainder of the string after the dimension
has been extracted
@return: true if it was possible to extract the dimension and false otherwise
*/
bool extractQRCodeDimensionFromString(const std::string& inputQRCodeString,
																			double& inputDimensionBuffer,
																			std::string& inputIdentifierBuffer) {
	...
	int minimumUnitIdentifierStartingIndex =
			buffer.size();  // The first index of the string that matches one of the
											// unit IDs
	std::string minimumUnitIDType;  // The unit ID that generated the minimum

	// Find which identifier (if any) to use
	for (auto iter = unitIdentifierToMetricMeterConversionFactor.begin();
			 iter != unitIdentifierToMetricMeterConversionFactor.end(); iter++) {
	...
	std::string numberPortion =
			buffer.substr(0, minimumUnitIdentifierStartingIndex);
	...
	inputDimensionBuffer =
			dimensionInOriginalUnits *
			unitIdentifierToMetricMeterConversionFactor.at(minimumUnitIDType);

	// Store the remainder of the string
	inputIdentifierBuffer = inputQRCodeString.substr(
			minimumUnitIdentifierStartingIndex + minimumUnitIDType.size(),
			std::string::npos);
